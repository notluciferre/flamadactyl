...
export function Safari({
  ...
  // Build responsive svg attributes: if width is a number and height is number, set both.
  // If width is a string like '100%', set width attribute and omit height so SVG scales preserving viewBox ratio.
  const svgWidthAttr = width ?? '100%'
  const svgHeightAttr = height ?? undefined
  ...
  return (
    <svg
      ...
      {...(props as any)}
      {...(svgWidthAttr !== undefined ? { width: svgWidthAttr } : {})}
      {...(svgHeightAttr !== undefined ? { height: svgHeightAttr } : {})}
    >
      <g clipPath="url(#path0)">
        <path
          className={imageSrc ? "fill-transparent dark:fill-transparent" : "fill-[#E5E5E5] dark:fill-[#404040]"}
